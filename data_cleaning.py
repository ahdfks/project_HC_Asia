...
df.at[df.index[1082], 'founded_on'] = 2018 #fix a companys' found yr:1

df['age_of_company'] = df.founded_on.dropna().apply(lambda x: datetime.now().year-x)

#look over types of ownership
df['type'].unique()
...
df1['therapeutic_area'] = df1['specialities'].apply(classify)
...
df1_out.to_csv('therapeutic_areas.csv', float_format="%.0f",index=False)
